...
inFile = read.table(input_file_name, sep="\t" ,header = header_row, row.names = header_col)
# Color
paletteLength <- 300
...
for (i in 1:length(list_cl_method)){
  for (j in 1:length(list_cl_dist)){
    ...
    outFile = paste0(output_file_name, "_", list_cl_method[i], "_", list_cl_dist[j], "_", norm_method_col)
